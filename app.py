...
def parse_int_paid_projected_universal(lines):
    """Exact parser for Int Paid/Projected Report (depd637t-style).

    One row per deposit account. Customer metadata and customer-level totals
    are carried onto each account row, while letter/footer text is excluded
    from Clean Data and retained verbatim in Raw Source.
    """
    u = "\n".join(lines).upper()
    if "INT PAID/PROJECTED REPORT" not in u:
        return None
    if "ACCOUNT NO." not in u or "INT-PROJECTED" not in u or "TDS APPLICABLE" not in u:
        return None

    rows = []
    report_id = proc_date = branch_code = branch_name = report_date = bank = ""
    current_name = current_address = current_post_cd = current_tel = ""
    current_birth_date = current_pan = current_15g = current_15h = ""
    current_subject = financial_year = ""
    customer_row_indices = []

    # Customer totals: PRINCIPAL, INT-PAID, TAX-DEDUCTED, INT-PROJECTED
    cust_total_principal = cust_total_int_paid = cust_total_tax = cust_total_projected = ""
    total_interest_paid = total_tax_deducted = ""

    i = 0
    while i < len(lines):
        line = lines[i].rstrip("\r\n")

        m = re.search(r"REPORT ID\s*:\s*(\S+)", line, re.I)
        if m:
            report_id = m.group(1).strip()
        m = re.search(r"PROC DATE\s*:\s*(\d{2}/\d{2}/\d{4})", line, re.I)
        if m:
            proc_date = m.group(1)
        m = re.search(r"BRANCH CODE\s*:\s*(\d+)\s+Int Paid/Projected Report", line, re.I)
        if m:
            branch_code = m.group(1).strip()
        m = re.search(r"\bBANK\s*:\s*(.+?)\s*,?\s*$", line, re.I)
        if m:
            bank = clean_text(m.group(1)).rstrip(",")
        m = re.search(r"\bBRANCH\s*:\s*\((\d+)\)\s*(.+?)\s*$", line, re.I)
        if m:
            branch_name = clean_text(m.group(2))
            if not branch_code:
                branch_code = m.group(1)
        m = re.search(r"\bDATE\s*:\s*(\d{2}/\d{2}/\d{4})", line, re.I)
        if m:
            report_date = m.group(1)

        mc = re.match(r"^\s*((?:MR|MRS|MS|MISS)\.?\s+.+?)\s*$", line, re.I)
        if mc and "REPORT ID" not in line.upper():
            current_name = clean_text(mc.group(1))
            # New customer: previous customer's summary values must not leak.
            cust_total_principal = cust_total_int_paid = ""
            cust_total_tax = cust_total_projected = ""
            total_interest_paid = total_tax_deducted = ""
            customer_row_indices = []
            ...
            j = i + 1
            while j < len(lines):
                s = clean_text(lines[j])
                if not s:
                    j += 1
                    continue
                if re.search(r"BIRTH DATE\s*:", s, re.I) or re.search(r"PAN NO\s*:", s, re.I):
                    break
                if re.match(r"^REPORT ID\s*:", s, re.I) or s.upper().startswith("DEAR "):
                    break
                addr.append(s)
                j += 1
            address_parts = []
            ...
            current_tel = ""
            for a in addr:
                if re.fullmatch(r"\d{6,8}", a):
                    current_post_cd = a
                elif re.fullmatch(r"\d{10}", a):
                    current_tel = a
                else:
                    address_parts.append(a)
            current_address = " | ".join(address_parts)

        m = re.search(r"BIRTH DATE\s*:-\s*(.+?)\s*$", line, re.I)
        if m:
            current_birth_date = clean_text(m.group(1))

        m = re.search(
            r"PAN NO\s*:\s*(.*?)\s+FORM 15G\s*:\s*([YN]?)\s+FORM 15H\s*:\s*([YN]?)\s*$",
            line, re.I
        )
        if m:
            current_pan = clean_text(m.group(1))
            current_15g = clean_text(m.group(2))
            current_15h = clean_text(m.group(3))

        m = re.search(r"SUBJECT\s*:\s*(\d+)", line, re.I)
        if m:
            current_subject = m.group(1)

        m = re.search(r"FINANCIAL YEAR\s*\(([^)]+)\)", line, re.I)
        if m:
            financial_year = clean_text(m.group(1))

        # Customer total line: preserve all four summary amounts.
        if "CUSTOMER TOTAL" in line.upper():
            nums = re.findall(r"-?[\d,]+\.\d+", line)
            if len(nums) >= 4:
                cust_total_principal = clean_amount(nums[-4])
                cust_total_int_paid = clean_amount(nums[-3])
                cust_total_tax = clean_amount(nums[-2])
                cust_total_projected = clean_amount(nums[-1])
                for idx in customer_row_indices:
                    rows[idx][-6:-2] = [
                        cust_total_principal, cust_total_int_paid,
                        cust_total_tax, cust_total_projected
                    ]
        ...
        m = re.search(r"TOTAL INTEREST PAID / ACCRUED\s*:\s*(-?[\d,]+\.\d+)", line, re.I)
        if m:
            total_interest_paid = clean_amount(m.group(1))
            for idx in customer_row_indices:
                rows[idx][-2] = total_interest_paid
            i += 1
            continue

        m = re.search(r"TOTAL TAX DEDUCTED\s*:\s*(-?[\d,]+\.\d+)", line, re.I)
        if m:
            total_tax_deducted = clean_amount(m.group(1))
            for idx in customer_row_indices:
                rows[idx][-1] = total_tax_deducted
            i += 1
            continue
        ...
        if ma:
            row = [
                report_id, proc_date, branch_code, branch_name, bank,
                report_date, current_name, current_address, current_post_cd,
                current_tel, current_birth_date, current_pan, current_15g,
                current_15h, current_subject, financial_year,
                ma.group(1), ma.group(2), ma.group(3), clean_amount(ma.group(4)),
                clean_text(ma.group(5)), clean_amount(ma.group(6)),
                clean_amount(ma.group(7)), clean_amount(ma.group(8)),
                clean_text(ma.group(9)), clean_text(ma.group(10)),
                clean_text(ma.group(11)),
                cust_total_principal, cust_total_int_paid,
                cust_total_tax, cust_total_projected,
                total_interest_paid, total_tax_deducted
            ]
            rows.append(row)
            customer_row_indices.append(len(rows) - 1)
    ...
    return pd.DataFrame(rows, columns=[
        "REPORT ID", "PROC DATE", "BRANCH CODE", "BRANCH NAME", "BANK",
        "REPORT DATE", "CUSTOMER NAME", "CUSTOMER ADDRESS", "CUSTOMER POST CODE",
        "TEL NO", "BIRTH DATE", "PAN NO", "FORM 15G", "FORM 15H",
        "CUSTOMER NO", "FINANCIAL YEAR",
        "ACCOUNT NO", "DEP DATE", "MAT DATE", "PRINCIPAL AMT", "RATE",
        "INT PAID", "TAX DEDUCTED", "INT PROJECTED", "ACC STATUS",
        "DESCRIPTION", "TDS APPLICABLE",
        "CUSTOMER TOTAL PRINCIPAL", "CUSTOMER TOTAL INT PAID",
        "CUSTOMER TOTAL TAX", "CUSTOMER TOTAL INT PROJECTED",
        "TOTAL INTEREST PAID / ACCRUED", "TOTAL TAX DEDUCTED"
    ])
...
def parse_universal_report(text):
    ...
    parsers=[
        (parse_int_paid_projected_universal,"INT_PAID_PROJECTED"),
